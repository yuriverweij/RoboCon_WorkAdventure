...
The Gather version of the RoboCon 2026 central venue has been ported to
[`robocon26-native.tmj`](./robocon26-native.tmj). It includes:

- all 21 original Gather tile layers, in their original order;
- a WorkAdventure start area at the bottom-centre entrance;
- native collision tiles inferred from the source walls, furniture, lounges,
  hall seating, and screen layers.

### Daily Tiled workflow

`robocon26-native.tmj` is the editable source of truth for WorkAdventure. Open
that file directly in Tiled, edit it, and press **Save**. Do not export it and do
not rerun the Gather converter for normal map editing.

Run `npm start` once and load:

```text
https://play.workadventu.re/_/6onm6kqkhnc/127.0.0.1:5173/robocon26-native.tmj
```

After each save, reload WorkAdventure. The development server disables caching,
so the saved `.tmj` and its referenced PNG tilesets are requested again. To edit
the collision layer, temporarily raise its opacity in Tiled so the `BLOCK` tiles
are visible, paint or erase tiles, then return its opacity to 0 before saving.

The generated layer atlases under `robocon/robocon26-native-layers/` are
dependencies of the map and should remain beside it, but they do not need to be
regenerated or exported after Tiled edits. The converter below is retained only
as a one-time re-import tool for the original Gather map; running it overwrites
the editable WorkAdventure map.

### One-time Gather import

The Gather project uses a 16 px grid, while WorkAdventure uses a 32 px world
grid and 32 px player sprites.
[`scripts/convert-gather-map-layers.ps1`](./scripts/convert-gather-map-layers.ps1)
preserves each source layer independently, including its name, order, offset,
visibility and opacity. Each 2x2 block of Gather cells is repacked into native
32 px composite tiles without resampling the pixels. Run the one-time import
with:

```powershell
& .\scripts\convert-gather-map-layers.ps1 `
  -SourceMap '..\RoboConGather-1\Maps\RBCN26.tmx' `
  -PreviewImage '..\RoboConGather-1\Maps\RBCN26_BG.png' `
  -OutputMap '.\robocon26-native.tmj'
```

The five source layers below the `------------------` separator remain hidden,
matching the original TMX. Enable the foreground layers you want in Tiled and
give avatar-occluding layers an `above...` name before saving.

Gather portal destinations and livestream URLs are hosted configuration rather
than data in the source Tiled project. Add those as WorkAdventure areas once
the destination map and current event URLs are known.
